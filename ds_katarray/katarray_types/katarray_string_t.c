...
size_t katarray_string_set_overwrite_at(katarray_string_t **KatArray, ssize_t index, char* value);
void *malloc_wrapper(size_t size, const char* function_name);

// prints all elements
// if option > 0: then display index.
void katarray_string_length_print(katarray_string_t *KatArray, char* option) {
    if (option > 0) {
        // index
        printf(GRAY" ");
...
void katarray_string_capacity_print(katarray_string_t *KatArray, char option) {
    if (option > 0) {
        // index
        printf(GRAY" ");
...
void katarray_string_debug_print(katarray_string_t *KatArray, char option) {
    if (option > 0) {
...
// create arraylist
katarray_string_t *katarray_string_create(size_t length, char* initial_value, size_t capacity, char shrinkable) {
    if (length > capacity) {
        length = capacity;
    }
    if (capacity <= 0) {
    ...
    katarray_string_t *KatArray = malloc_wrapper(sizeof(*KatArray), __func__);
...
// free arraylist
void katarray_string_free(katarray_string_t *KatArray) {
    free(KatArray->list);
    free(KatArray);
...
// resize arraylist
void katarray_string_resize(katarray_string_t **KatArray, double growth_factor, size_t add_factor) {

    size_t new_capacity = (*KatArray)->capacity * growth_factor + add_factor;

    // initialize new arraylist
    katarray_string_t *New_ArrayList = katarray_string_create((*KatArray)->length, (*KatArray)->initial_value, new_capacity, (*KatArray)->shrinkable);
...
void katarray_string_convert(katarray_string_t **KatArray) {

    katarray_string_t *New_ArrayList = katarray_string_create((*KatArray)->length, (*KatArray)->initial_value, (*KatArray)->capacity, (*KatArray)->shrinkable);
...
// set end
size_t katarray_string_set_append(katarray_string_t **KatArray, char* value) {

    // check if resize needed
    if ((*KatArray)->length == (*KatArray)->capacity) {
        katarray_string_resize(KatArray, 2, 0);
    }
...
// set start
size_t katarray_string_set_prepend(katarray_string_t **KatArray, char* value) {
...
// set insert at
ssize_t katarray_string_set_insert_at(katarray_string_t **KatArray, ssize_t index, char* value) {
    if ((size_t)index < (*KatArray)->length && index >= 0) {

        // check if resize needed
        if ((*KatArray)->length == (*KatArray)->capacity) {
            katarray_string_resize(KatArray, 2, index);
        }
    ...
    // do katarray_string_set_overwrite_at
    else {
        katarray_string_set_overwrite_at(KatArray, index, value);
    }
...
size_t katarray_string_set_overwrite_at(katarray_string_t **KatArray, ssize_t index, char* value) {
    ...
    // check if negative
        ...
        if ((size_t)(index * -1) + (*KatArray)->length > (*KatArray)->capacity) {
            katarray_string_resize(KatArray, 2, index * -1);
        }
...
// remove appended
char* katarray_string_remove_append(katarray_string_t **KatArray) {
    ...
    if ((*KatArray)->shrinkable > 0 && (*KatArray)->capacity > 5 && (*KatArray)->length <= ((*KatArray)->capacity / 3)) {
        katarray_string_resize(KatArray, 0.5f, 0);
    }
...
// returns the removed value
// remove prepended
char* katarray_string_remove_prepend(katarray_string_t **KatArray) {
...
// remove insert at
char* katarray_string_remove_insert_at(katarray_string_t **KatArray, size_t index) {
...
char* katarray_string_remove_overwrite_at(katarray_string_t **KatArray, size_t index) {
    
    char* removed_value = (*KatArray)->list[(index + (*KatArray)->index_start) % (*KatArray)->capacity];

    // if at start or end, reroute
    if (index == 0) {
        katarray_string_remove_prepend(KatArray);
    }
    else if (index >= (*KatArray)->length - 1) {
        katarray_string_remove_append(KatArray);
    }
...
void katarray_string_reset(katarray_string_t **KatArray, size_t length, size_t capacity) {
    *KatArray = katarray_string_create(length, (*KatArray)->initial_value, capacity, (*KatArray)->shrinkable);
    return;
}
...
// get value at index
char* katarray_string_get_value_at(katarray_string_t *KatArray, size_t index) {
    if (index < KatArray->length) {
...
char* katarray_string_get_first_value(katarray_string_t *KatArray) {
    return KatArray->list[KatArray->index_start];
}

// get value at end index
char* katarray_string_get_last_value(katarray_string_t *KatArray) {
    return KatArray->list[KatArray->index_end];
}

// get length of array
size_t katarray_string_get_length(katarray_string_t *KatArray) {
    return KatArray->length;
}

// get capacity of array
size_t katarray_string_get_capacity(katarray_string_t *KatArray) {
...
// enqueue
size_t katarray_string_enqueue(katarray_string_t **KatArray, char* value) {
    return katarray_string_set_append(KatArray, value);
}

// dequeue
char* katarray_string_dequeue(katarray_string_t **KatArray) {
    return katarray_string_remove_prepend(KatArray);
}

// peek at the start/head of the arraylist
char* katarray_string_peek_head(katarray_string_t *KatArray) {
    return katarray_string_get_first_value(KatArray);
}
...
char katarray_string_is_full(katarray_string_t *KatArray) {
    if (KatArray->length == KatArray->capacity) {
        return 1;
    }
    else return -1;
...
// is empty
char katarray_string_is_empty(katarray_string_t *KatArray) {
...
// push
size_t katarray_string_push(katarray_string_t **KatArray, char* value) {
    return katarray_string_set_append(KatArray, value);
}

// pop
char* katarray_string_pop(katarray_string_t **KatArray) {
    return katarray_string_remove_append(KatArray);
}

// peek top
char* katarray_string_peek_top(katarray_string_t *KatArray) {
    return katarray_string_get_last_value(KatArray);
}
