...
void katarray_longdouble_length_print(katarray_longdouble_t *KatArray, long double option) {
    if (option > 0) {
        // index
        printf(GRAY" ");
        ...
        printf("[");
        for (size_t i = 0; i < KatArray->length; i++) {
            printf(" %.10Lg", KatArray->list[(KatArray->index_start + i) % KatArray->capacity]);
            if (i < KatArray->length - 1) {
                printf(",");
            }
        }
    ...
    if (option > 0) {
        ...
        for (size_t i = 0; i < KatArray->capacity; i++) {
            ...
            if (i == index_start && i == index_end) {
                fprintf(stderr, ORANGE" %.10Lg"END, KatArray->list[i]);
            }
            else if (i == index_start) {
                fprintf(stderr, GREEN" %.10Lg"END, KatArray->list[i]);
            }
            else if (i == index_end) {
                fprintf(stderr, RED" %.10Lg"END, KatArray->list[i]);
            }
            else {
                fprintf(stderr, " %.10Lg", KatArray->list[i]);
            }
...
katarray_longdouble_t *katarray_longdouble_create(size_t length, long double initial_value, size_t capacity, char shrinkable) {
    ...
    katarray_longdouble_t *KatArray = malloc_wrapper(sizeof(*KatArray), __func__);
...
void katarray_longdouble_resize(katarray_longdouble_t **KatArray, double growth_factor, size_t add_factor) {
    ...
    katarray_longdouble_t *New_ArrayList = katarray_longdouble_create((*KatArray)->length, (*KatArray)->initial_value, new_capacity, (*KatArray)->shrinkable);
...
void katarray_longdouble_convert(katarray_longdouble_t **KatArray) {
    ...
    katarray_longdouble_t *New_ArrayList = katarray_longdouble_create((*KatArray)->length, (*KatArray)->initial_value, (*KatArray)->capacity, (*KatArray)->shrinkable);
...
size_t katarray_longdouble_set_append(katarray_longdouble_t **KatArray, long double value) {
    ...
    if ((*KatArray)->length == (*KatArray)->capacity) {
        katarray_longdouble_resize(KatArray, 2, 0);
    }
...
// set insert at
ssize_t katarray_longdouble_set_insert_at(katarray_longdouble_t **KatArray, ssize_t index, long double value) {
    if ((size_t)index < (*KatArray)->length && index >= 0) {

        // check if resize needed
        if ((*KatArray)->length == (*KatArray)->capacity) {
            katarray_longdouble_resize(KatArray, 2, index);
        }

        // sliding window
        // insert numbers, shove numbers to the right
        long double temp_out         = value;
    ...
    // do katarray_longdouble_set_overwrite_at
    else {
        katarray_longdouble_set_overwrite_at(KatArray, index, value);
    }
...
size_t katarray_longdouble_set_overwrite_at(katarray_longdouble_t **KatArray, ssize_t index, long double value) {
    ...
    // check if negative
        ...
        if ((size_t)(index * -1) + (*KatArray)->length > (*KatArray)->capacity) {
            katarray_longdouble_resize(KatArray, 2, index * -1);
        }
...
long double katarray_longdouble_remove_prepend(katarray_longdouble_t **KatArray) {
    ...
    long double removed_value = (*KatArray)->list[(*KatArray)->index_start];
...
long double katarray_longdouble_remove_insert_at(katarray_longdouble_t **KatArray, size_t index) {
    ...
    long double removed_value = (*KatArray)->list[(index + (*KatArray)->index_start) % (*KatArray)->capacity];
...
long double katarray_longdouble_remove_overwrite_at(katarray_longdouble_t **KatArray, size_t index) {
    ...
    if (index == 0) {
        katarray_longdouble_remove_prepend(KatArray);
    }
    else if (index >= (*KatArray)->length - 1) {
        katarray_longdouble_remove_append(KatArray);
    }
...
long double katarray_longdouble_pop(katarray_longdouble_t **KatArray) {
    return katarray_longdouble_remove_append(KatArray);
}

// peek top
long double katarray_longdouble_peek_top(katarray_longdouble_t *KatArray) {
    return katarray_longdouble_get_last_value(KatArray);
}
