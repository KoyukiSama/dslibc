...
void katarray_int_length_print(katarray_int_t *KatArray, int option) {
    if (option > 0) {
        // index
        printf(GRAY" ");
        ...
        printf("[");
        for (size_t i = 0; i < KatArray->length; i++) {
            printf(" %i", KatArray->list[(KatArray->index_start + i) % KatArray->capacity]);
            if (i < KatArray->length - 1) {
                printf(",");
            }
        }
    ...
    if (option > 0) {
        ...
        for (size_t i = 0; i < KatArray->capacity; i++) {
            ...
            if (i == index_start && i == index_end) {
                fprintf(stderr, ORANGE" %ig"END, KatArray->list[i]);
            }
            else if (i == index_start) {
                fprintf(stderr, GREEN" %ig"END, KatArray->list[i]);
            }
            else if (i == index_end) {
                fprintf(stderr, RED" %ig"END, KatArray->list[i]);
            }
            else {
                fprintf(stderr, " %ig", KatArray->list[i]);
            }
...
katarray_int_t *katarray_int_create(size_t length, int initial_value, size_t capacity, char shrinkable) {
    ...
    katarray_int_t *KatArray = malloc_wrapper(sizeof(*KatArray), __func__);
...
void katarray_int_resize(katarray_int_t **KatArray, double growth_factor, size_t add_factor) {
    ...
    katarray_int_t *New_ArrayList = katarray_int_create((*KatArray)->length, (*KatArray)->initial_value, new_capacity, (*KatArray)->shrinkable);
...
void katarray_int_convert(katarray_int_t **KatArray) {
    ...
    katarray_int_t *New_ArrayList = katarray_int_create((*KatArray)->length, (*KatArray)->initial_value, (*KatArray)->capacity, (*KatArray)->shrinkable);
...
size_t katarray_int_set_append(katarray_int_t **KatArray, int value) {
    ...
    if ((*KatArray)->length == (*KatArray)->capacity) {
        katarray_int_resize(KatArray, 2, 0);
    }
...
// set insert at
ssize_t katarray_int_set_insert_at(katarray_int_t **KatArray, ssize_t index, int value) {
    if ((size_t)index < (*KatArray)->length && index >= 0) {

        // check if resize needed
        if ((*KatArray)->length == (*KatArray)->capacity) {
            katarray_int_resize(KatArray, 2, index);
        }
    ...
    // do katarray_int_set_overwrite_at
    else {
        katarray_int_set_overwrite_at(KatArray, index, value);
    }
...
size_t katarray_int_set_overwrite_at(katarray_int_t **KatArray, ssize_t index, int value) {
    ...
    // check if negative
        ...
        if ((size_t)(index * -1) + (*KatArray)->length > (*KatArray)->capacity) {
            katarray_int_resize(KatArray, 2, index * -1);
        }
...
int katarray_int_remove_prepend(katarray_int_t **KatArray) {
    ...
    int removed_value = (*KatArray)->list[(*KatArray)->index_start];
...
int katarray_int_remove_insert_at(katarray_int_t **KatArray, size_t index) {
    ...
    int removed_value = (*KatArray)->list[(index + (*KatArray)->index_start) % (*KatArray)->capacity];
...
int katarray_int_remove_overwrite_at(katarray_int_t **KatArray, size_t index) {
    ...
    if (index == 0) {
        katarray_int_remove_prepend(KatArray);
    }
    else if (index >= (*KatArray)->length - 1) {
        katarray_int_remove_append(KatArray);
    }
...
int katarray_int_pop(katarray_int_t **KatArray) {
    return katarray_int_remove_append(KatArray);
}

// peek top
int katarray_int_peek_top(katarray_int_t *KatArray) {
    return katarray_int_get_last_value(KatArray);
}
