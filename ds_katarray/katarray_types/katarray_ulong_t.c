...
void katarray_ulong_length_prlong(katarray_ulong_t *KatArray, unsigned long option) {
    if (option > 0) {
        // index
        prlongf(GRAY" ");
        ...
        prlongf("[");
        for (size_t i = 0; i < KatArray->length; i++) {
            prlongf(" %lu", KatArray->list[(KatArray->index_start + i) % KatArray->capacity]);
            if (i < KatArray->length - 1) {
                prlongf(",");
            }
        }
    ...
    if (option > 0) {
        ...
        for (size_t i = 0; i < KatArray->capacity; i++) {
            ...
            if (i == index_start && i == index_end) {
                fprlongf(stderr, ORANGE" %lu"END, KatArray->list[i]);
            }
            else if (i == index_start) {
                fprlongf(stderr, GREEN" %lu"END, KatArray->list[i]);
            }
            else if (i == index_end) {
                fprlongf(stderr, RED" %lu"END, KatArray->list[i]);
            }
            else {
                fprlongf(stderr, " %lu", KatArray->list[i]);
            }
...
katarray_ulong_t *katarray_ulong_create(size_t length, unsigned long initial_value, size_t capacity, char shrinkable) {
    ...
    katarray_ulong_t *KatArray = malloc_wrapper(sizeof(*KatArray), __func__);
...
void katarray_ulong_resize(katarray_ulong_t **KatArray, double growth_factor, size_t add_factor) {
    ...
    katarray_ulong_t *New_ArrayList = katarray_ulong_create((*KatArray)->length, (*KatArray)->initial_value, new_capacity, (*KatArray)->shrinkable);
...
void katarray_ulong_convert(katarray_ulong_t **KatArray) {
    ...
    katarray_ulong_t *New_ArrayList = katarray_ulong_create((*KatArray)->length, (*KatArray)->initial_value, (*KatArray)->capacity, (*KatArray)->shrinkable);
...
size_t katarray_ulong_set_append(katarray_ulong_t **KatArray, unsigned long value) {
    ...
    if ((*KatArray)->length == (*KatArray)->capacity) {
        katarray_ulong_resize(KatArray, 2, 0);
    }
...
// set insert at
ssize_t katarray_ulong_set_insert_at(katarray_ulong_t **KatArray, ssize_t index, unsigned long value) {
    if ((size_t)index < (*KatArray)->length && index >= 0) {

        // check if resize needed
        if ((*KatArray)->length == (*KatArray)->capacity) {
            katarray_ulong_resize(KatArray, 2, index);
        }

        // sliding window
        // insert numbers, shove numbers to the right
        unsigned long temp_out         = value;
        unsigned long temp_in          = 0;
        (*KatArray)->length += 1;
    ...
    // do katarray_ulong_set_overwrite_at
    else {
        katarray_ulong_set_overwrite_at(KatArray, index, value);
    }
...
size_t katarray_ulong_set_overwrite_at(katarray_ulong_t **KatArray, ssize_t index, unsigned long value) {
    ...
    // check if negative
        ...
        if ((size_t)(index * -1) + (*KatArray)->length > (*KatArray)->capacity) {
            katarray_ulong_resize(KatArray, 2, index * -1);
        }
...
unsigned long katarray_ulong_remove_prepend(katarray_ulong_t **KatArray) {
    ...
    unsigned long removed_value = (*KatArray)->list[(*KatArray)->index_start];
...
unsigned long katarray_ulong_remove_insert_at(katarray_ulong_t **KatArray, size_t index) {
    ...
    unsigned long removed_value = (*KatArray)->list[(index + (*KatArray)->index_start) % (*KatArray)->capacity];
...
unsigned long katarray_ulong_remove_overwrite_at(katarray_ulong_t **KatArray, size_t index) {
    ...
    if (index == 0) {
        katarray_ulong_remove_prepend(KatArray);
    }
    else if (index >= (*KatArray)->length - 1) {
        katarray_ulong_remove_append(KatArray);
    }
...
unsigned long katarray_ulong_pop(katarray_ulong_t **KatArray) {
    return katarray_ulong_remove_append(KatArray);
}

// peek top
unsigned long katarray_ulong_peek_top(katarray_ulong_t *KatArray) {
    return katarray_ulong_get_last_value(KatArray);
}
