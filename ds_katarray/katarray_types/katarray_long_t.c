...
void katarray_long_length_print(katarray_long_t *KatArray, long option) {
    if (option > 0) {
        // index
        printf(GRAY" ");
        ...
        printf("[");
        for (size_t i = 0; i < KatArray->length; i++) {
            printf(" %ld", KatArray->list[(KatArray->index_start + i) % KatArray->capacity]);
            if (i < KatArray->length - 1) {
                printf(",");
            }
        }
    ...
    if (option > 0) {
        ...
        for (size_t i = 0; i < KatArray->capacity; i++) {
            ...
            if (i == index_start && i == index_end) {
                fprintf(stderr, ORANGE" %ld"END, KatArray->list[i]);
            }
            else if (i == index_start) {
                fprintf(stderr, GREEN" %ld"END, KatArray->list[i]);
            }
            else if (i == index_end) {
                fprintf(stderr, RED" %ld"END, KatArray->list[i]);
            }
            else {
                fprintf(stderr, " %ld", KatArray->list[i]);
            }
...
katarray_long_t *katarray_long_create(size_t length, long initial_value, size_t capacity, char shrinkable) {
    ...
    katarray_long_t *KatArray = malloc_wrapper(sizeof(*KatArray), __func__);
...
void katarray_long_resize(katarray_long_t **KatArray, double growth_factor, size_t add_factor) {
    ...
    katarray_long_t *New_ArrayList = katarray_long_create((*KatArray)->length, (*KatArray)->initial_value, new_capacity, (*KatArray)->shrinkable);
...
void katarray_long_convert(katarray_long_t **KatArray) {
    ...
    katarray_long_t *New_ArrayList = katarray_long_create((*KatArray)->length, (*KatArray)->initial_value, (*KatArray)->capacity, (*KatArray)->shrinkable);
...
size_t katarray_long_set_append(katarray_long_t **KatArray, long value) {
    ...
    if ((*KatArray)->length == (*KatArray)->capacity) {
        katarray_long_resize(KatArray, 2, 0);
    }
...
// set insert at
ssize_t katarray_long_set_insert_at(katarray_long_t **KatArray, ssize_t index, long value) {
    if ((size_t)index < (*KatArray)->length && index >= 0) {

        // check if resize needed
        if ((*KatArray)->length == (*KatArray)->capacity) {
            katarray_long_resize(KatArray, 2, index);
        }
    ...
    // do katarray_long_set_overwrite_at
    else {
        katarray_long_set_overwrite_at(KatArray, index, value);
    }
...
size_t katarray_long_set_overwrite_at(katarray_long_t **KatArray, ssize_t index, long value) {
    ...
    // check if negative
        ...
        if ((size_t)(index * -1) + (*KatArray)->length > (*KatArray)->capacity) {
            katarray_long_resize(KatArray, 2, index * -1);
        }
...
long katarray_long_remove_prepend(katarray_long_t **KatArray) {
    ...
    long removed_value = (*KatArray)->list[(*KatArray)->index_start];
...
long katarray_long_remove_insert_at(katarray_long_t **KatArray, size_t index) {
    ...
    long removed_value = (*KatArray)->list[(index + (*KatArray)->index_start) % (*KatArray)->capacity];
...
long katarray_long_remove_overwrite_at(katarray_long_t **KatArray, size_t index) {
    ...
    if (index == 0) {
        katarray_long_remove_prepend(KatArray);
    }
    else if (index >= (*KatArray)->length - 1) {
        katarray_long_remove_append(KatArray);
    }
...
long katarray_long_pop(katarray_long_t **KatArray) {
    return katarray_long_remove_append(KatArray);
}

// peek top
long katarray_long_peek_top(katarray_long_t *KatArray) {
    return katarray_long_get_last_value(KatArray);
}
