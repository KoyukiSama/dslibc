...
void katarray_double_length_print(katarray_double_t *KatArray, double option) {
    if (option > 0) {
        // index
        printf(GRAY" ");
...
katarray_double_t *katarray_double_create(size_t length, double initial_value, size_t capacity, char shrinkable) {
    ...
    katarray_double_t *KatArray = malloc_wrapper(sizeof(*KatArray), __func__);
...
void katarray_double_resize(katarray_double_t **KatArray, double growth_factor, size_t add_factor) {
    ...
    katarray_double_t *New_ArrayList = katarray_double_create((*KatArray)->length, (*KatArray)->initial_value, new_capacity, (*KatArray)->shrinkable);
...
void katarray_double_convert(katarray_double_t **KatArray) {
    ...
    katarray_double_t *New_ArrayList = katarray_double_create((*KatArray)->length, (*KatArray)->initial_value, (*KatArray)->capacity, (*KatArray)->shrinkable);
...
size_t katarray_double_set_append(katarray_double_t **KatArray, double value) {
    ...
    if ((*KatArray)->length == (*KatArray)->capacity) {
        katarray_double_resize(KatArray, 2, 0);
    }
...
// set insert at
ssize_t katarray_double_set_insert_at(katarray_double_t **KatArray, ssize_t index, double value) {
    if ((size_t)index < (*KatArray)->length && index >= 0) {

        // check if resize needed
        if ((*KatArray)->length == (*KatArray)->capacity) {
            katarray_double_resize(KatArray, 2, index);
        }

        // sliding window
        // insert numbers, shove numbers to the right
        double temp_out         = value;
        double temp_in          = 0;
        (*KatArray)->length += 1;
    ...
    // do katarray_double_set_overwrite_at
    else {
        katarray_double_set_overwrite_at(KatArray, index, value);
    }
...
size_t katarray_double_set_overwrite_at(katarray_double_t **KatArray, ssize_t index, double value) {
    ...
    // check if negative
        ...
        if ((size_t)(index * -1) + (*KatArray)->length > (*KatArray)->capacity) {
            katarray_double_resize(KatArray, 2, index * -1);
        }
...
double katarray_double_remove_prepend(katarray_double_t **KatArray) {
    ...
    double removed_value = (*KatArray)->list[(*KatArray)->index_start];
...
double katarray_double_remove_insert_at(katarray_double_t **KatArray, size_t index) {
    ...
    double removed_value = (*KatArray)->list[(index + (*KatArray)->index_start) % (*KatArray)->capacity];
...
double katarray_double_remove_overwrite_at(katarray_double_t **KatArray, size_t index) {
    ...
    if (index == 0) {
        katarray_double_remove_prepend(KatArray);
    }
    else if (index >= (*KatArray)->length - 1) {
        katarray_double_remove_append(KatArray);
    }
...
double katarray_double_pop(katarray_double_t **KatArray) {
    return katarray_double_remove_append(KatArray);
}

// peek top
double katarray_double_peek_top(katarray_double_t *KatArray) {
    return katarray_double_get_last_value(KatArray);
}
