...
void katarray_uint_length_print(katarray_uint_t *KatArray, unsigned int option) {
    if (option > 0) {
        // index
        printf(GRAY" ");
        ...
        printf("[");
        for (size_t i = 0; i < KatArray->length; i++) {
            printf(" %u", KatArray->list[(KatArray->index_start + i) % KatArray->capacity]);
            if (i < KatArray->length - 1) {
                printf(",");
            }
        }
    ...
    if (option > 0) {
        ...
        for (size_t i = 0; i < KatArray->capacity; i++) {
            ...
            if (i == index_start && i == index_end) {
                fprintf(stderr, ORANGE" %u"END, KatArray->list[i]);
            }
            else if (i == index_start) {
                fprintf(stderr, GREEN" %u"END, KatArray->list[i]);
            }
            else if (i == index_end) {
                fprintf(stderr, RED" %u"END, KatArray->list[i]);
            }
            else {
                fprintf(stderr, " %u", KatArray->list[i]);
            }
...
katarray_uint_t *katarray_uint_create(size_t length, unsigned int initial_value, size_t capacity, char shrinkable) {
    ...
    katarray_uint_t *KatArray = malloc_wrapper(sizeof(*KatArray), __func__);
...
void katarray_uint_resize(katarray_uint_t **KatArray, double growth_factor, size_t add_factor) {
    ...
    katarray_uint_t *New_ArrayList = katarray_uint_create((*KatArray)->length, (*KatArray)->initial_value, new_capacity, (*KatArray)->shrinkable);
...
void katarray_uint_convert(katarray_uint_t **KatArray) {
    ...
    katarray_uint_t *New_ArrayList = katarray_uint_create((*KatArray)->length, (*KatArray)->initial_value, (*KatArray)->capacity, (*KatArray)->shrinkable);
...
size_t katarray_uint_set_append(katarray_uint_t **KatArray, unsigned int value) {
    ...
    if ((*KatArray)->length == (*KatArray)->capacity) {
        katarray_uint_resize(KatArray, 2, 0);
    }
...
// set insert at
ssize_t katarray_uint_set_insert_at(katarray_uint_t **KatArray, ssize_t index, unsigned int value) {
    if ((size_t)index < (*KatArray)->length && index >= 0) {

        // check if resize needed
        if ((*KatArray)->length == (*KatArray)->capacity) {
            katarray_uint_resize(KatArray, 2, index);
        }

        // sliding window
        // insert numbers, shove numbers to the right
        unsigned int temp_out         = value;
    ...
    // do katarray_uint_set_overwrite_at
    else {
        katarray_uint_set_overwrite_at(KatArray, index, value);
    }
...
size_t katarray_uint_set_overwrite_at(katarray_uint_t **KatArray, ssize_t index, unsigned int value) {
    ...
    // check if negative
        ...
        if ((size_t)(index * -1) + (*KatArray)->length > (*KatArray)->capacity) {
            katarray_uint_resize(KatArray, 2, index * -1);
        }
...
unsigned int katarray_uint_remove_prepend(katarray_uint_t **KatArray) {
    ...
    unsigned int removed_value = (*KatArray)->list[(*KatArray)->index_start];
...
unsigned int katarray_uint_remove_insert_at(katarray_uint_t **KatArray, size_t index) {
    ...
    unsigned int removed_value = (*KatArray)->list[(index + (*KatArray)->index_start) % (*KatArray)->capacity];
...
unsigned int katarray_uint_remove_overwrite_at(katarray_uint_t **KatArray, size_t index) {
    ...
    if (index == 0) {
        katarray_uint_remove_prepend(KatArray);
    }
    else if (index >= (*KatArray)->length - 1) {
        katarray_uint_remove_append(KatArray);
    }
...
unsigned int katarray_uint_pop(katarray_uint_t **KatArray) {
    return katarray_uint_remove_append(KatArray);
}

// peek top
unsigned int katarray_uint_peek_top(katarray_uint_t *KatArray) {
    return katarray_uint_get_last_value(KatArray);
}
