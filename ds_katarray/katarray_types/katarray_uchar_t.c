...
void katarray_uchar_length_print(katarray_uchar_t *KatArray, unsigned char option) {
    if (option > 0) {
        // index
        printf(GRAY" ");
        ...
        printf("[");
        for (size_t i = 0; i < KatArray->length; i++) {
            printf(" %hhu", KatArray->list[(KatArray->index_start + i) % KatArray->capacity]);
            if (i < KatArray->length - 1) {
                printf(",");
            }
        }
    ...
    if (option > 0) {
        ...
        for (size_t i = 0; i < KatArray->capacity; i++) {
            ...
            if (i == index_start && i == index_end) {
                fprintf(stderr, ORANGE" %hhu"END, KatArray->list[i]);
            }
            else if (i == index_start) {
                fprintf(stderr, GREEN" %hhu"END, KatArray->list[i]);
            }
            else if (i == index_end) {
                fprintf(stderr, RED" %hhu"END, KatArray->list[i]);
            }
            else {
                fprintf(stderr, " %hhu", KatArray->list[i]);
            }
...
katarray_uchar_t *katarray_uchar_create(size_t length, unsigned char initial_value, size_t capacity, char shrinkable) {
    ...
    katarray_uchar_t *KatArray = malloc_wrapper(sizeof(*KatArray), __func__);
...
void katarray_uchar_resize(katarray_uchar_t **KatArray, double growth_factor, size_t add_factor) {
    ...
    katarray_uchar_t *New_ArrayList = katarray_uchar_create((*KatArray)->length, (*KatArray)->initial_value, new_capacity, (*KatArray)->shrinkable);
...
void katarray_uchar_convert(katarray_uchar_t **KatArray) {
    ...
    katarray_uchar_t *New_ArrayList = katarray_uchar_create((*KatArray)->length, (*KatArray)->initial_value, (*KatArray)->capacity, (*KatArray)->shrinkable);
...
size_t katarray_uchar_set_append(katarray_uchar_t **KatArray, unsigned char value) {
    ...
    if ((*KatArray)->length == (*KatArray)->capacity) {
        katarray_uchar_resize(KatArray, 2, 0);
    }
...
size_t katarray_uchar_set_insert_at(katarray_uchar_t **KatArray, ssize_t index, unsigned char value) {
    if ((size_t)index < (*KatArray)->length && index >= 0) {
        ...
        // check if resize needed
        if ((*KatArray)->length == (*KatArray)->capacity) {
            katarray_uchar_resize(KatArray, 2, index);
        }

        // sliding window
        // insert numbers, shove numbers to the right
        unsigned char temp_out         = value;
        unsigned char temp_in          = 0;
        (*KatArray)->length += 1;
    ...
    // do katarray_uchar_set_overwrite_at
    else {
        katarray_uchar_set_overwrite_at(KatArray, index, value);
    }
...
size_t katarray_uchar_set_overwrite_at(katarray_uchar_t **KatArray, ssize_t index, unsigned char value) {
    ...
    // check if negative
        ...
        if ((size_t)(index * -1) + (*KatArray)->length > (*KatArray)->capacity) {
            katarray_uchar_resize(KatArray, 2, index * -1);
        }
...
unsigned char katarray_uchar_remove_prepend(katarray_uchar_t **KatArray) {
    ...
    unsigned char removed_value = (*KatArray)->list[(*KatArray)->index_start];
...
unsigned char katarray_uchar_remove_insert_at(katarray_uchar_t **KatArray, size_t index) {
    ...
    unsigned char removed_value = (*KatArray)->list[(index + (*KatArray)->index_start) % (*KatArray)->capacity];
...
unsigned char katarray_uchar_remove_overwrite_at(katarray_uchar_t **KatArray, size_t index) {
    ...
    if (index == 0) {
        katarray_uchar_remove_prepend(KatArray);
    }
    else if (index >= (*KatArray)->length - 1) {
        katarray_uchar_remove_append(KatArray);
    }
...
unsigned char katarray_uchar_pop(katarray_uchar_t **KatArray) {
    return katarray_uchar_remove_append(KatArray);
}
